...
#'
#' @return `ch_ews()` returns a matrix that contains:
#'   1. `time`: the time index.
#'   2. `r.squared`: the R2 values of the regressed residuals.
#'   3. `critical.value`: the chi-square critical value based on the desired `alpha` level for 1 degree of freedom divided by the number of residuals used in the regression.
#'   4. `test.result`: logical. It indicates whether conditional heteroskedasticity was significant.
#'   5. `ar.fit.order`: the order of the specified autoregressive model-only informative if `optim` FALSE was selected.
#'
#' In addition, if \code{plot=TRUE}, \code{ch_ews} plots the original timeseries and the R2 where the level of significance is also indicated.
#'
#' @export
#' @importFrom grDevices dev.new rainbow topo.colors
#' @importFrom graphics abline contour grid hist layout legend lines mtext par
...
ch_ews <- function(timeseries, winsize = 10, alpha = 0.1, optim = TRUE, lags = 4,
    logtransform = FALSE, interpolate = FALSE, plot = FALSE) {
    ...
    # Log-transformation
    if (logtransform) ts.in <- log(ts.in + 1)
...
plot_ch_ews <- function(timeindex, tsin, sto) {
  par(mar = c(0, 4, 0, 1), oma = c(5, 1, 2, 1), mfrow = c(2, 1))
