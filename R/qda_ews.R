...
#' @param detrending the timeseries can be detrended/filtered prior to analysis. There are four options: \code{gaussian} filtering, \code{linear} detrending and \code{first_differencing}. Default is \code{no} detrending.
...
#' @param plot logical. If TRUE then a default plot is returned.
#'
#' @return
#' \code{qda_ews()} produces three plots.
#' 1. The first plot contains the original data, the detrending/filtering applied and the residuals (if selected), autocorrelation and variance. For each statistic trends are estimated by the nonparametric Kendall tau correlation.
#' 2. The second plot, returns a histogram of the distributions of the Kendall trend statistic for autocorrelation and variance estimated on the surrogated data. Vertical lines represent the level of significance, whereas the black dots the actual trend found in the time series.
#' 3. The third plot is the reconstructed potential landscape in 2D.
#' In addition, the function returns a list containing the output from the respective functions generic_RShiny (indicators); surrogates_RShiny (trends); movpotential_ews (potential analysis)
#'
#' @export
#'
...
qda_ews <- function(timeseries, param = NULL, winsize = 50, detrending = c("no",
    "gaussian", "linear", "first_diff"), bandwidth = NULL, boots = 100, s_level = 0.05,
...
generic_RShiny <- function(timeseries, winsize = 50, detrending = c("no", "gaussian",
    "linear", "first_diff"), bandwidth, logtransform, interpolate, AR_n = FALSE,
    powerspectrum = FALSE) {
    ...
    if (detrending == "no") {
        ...
        legend("bottomleft", "first_differenced", bty = "n")
    } else {
        limit <- max(c(max(abs(nsmY))))
...
surrogates_RShiny <- function(timeseries, winsize = 50, detrending = c("no", "gaussian",
    "linear", "first_diff"), bandwidth = NULL, boots = 100, s_level = 0.05, logtransform = FALSE,
    ...
    if (plot) plot_surrogates_RShiny(Ktauestind_ar1, Ktauestind_var,
      Ktauestind_varorig, Ktauestind_ar1orig, q_ar1, s_level, boots)

}


plot_surrogates_RShiny <- function(Ktauestind_ar1, Ktauestind_var,
  Ktauestind_varorig, Ktauestind_ar1orig, q_ar1, s_level, boots) {
  ...
  hist(Ktauestind_var, freq = TRUE, nclass = 20, xlim = c(-1, 1), col = "blue",
      main = NULL, xlab = "Surrogate trend estimates", ylab = "occurrence")
      #,ylim=c(0, boots))
