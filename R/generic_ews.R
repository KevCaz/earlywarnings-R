...
#' @param detrending the timeseries can be detrended/filtered prior to analysis. There are four options: \code{gaussian} filtering, \code{loess} fitting, \code{linear} detrending and \code{first_differencing}. Default is \code{no} detrending.
...
generic_ews <- function(timeseries, winsize = 50, detrending = c("no", "gaussian",
    "loess", "linear", "first_diff"), bandwidth = NULL, span = NULL, degree = NULL,
    ...
    if (detrending == "no") {
      ...
      # timeindexdiff <- timeindex[1:(length(timeindex) - 1)]
        ...
        legend("topleft", "first_differenced", bty = "n")
    } else {
        limit <- max(c(max(abs(nsmY))))
